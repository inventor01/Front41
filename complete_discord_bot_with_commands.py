...
class DatabaseManager:
    def __init__(self):
        # Force Railway database connection - this is the proven solution
...
@bot.tree.command(name="list_keywords", description="Show all monitored keywords")
async def list_keywords(interaction: discord.Interaction):
    try:
        ...
        cursor = conn.cursor()
        # Show ALL keywords from the system - not just user-specific ones
        ...
        if not keywords:
            embed = discord.Embed(
                ...
                description="No keywords found in database.",
                color=0x0099ff
            )
        else:
            keyword_list = "\n".join([f"• {kw[0]}" for kw in keywords[:20]])  # Show first 20
            if len(keywords) > 20:
